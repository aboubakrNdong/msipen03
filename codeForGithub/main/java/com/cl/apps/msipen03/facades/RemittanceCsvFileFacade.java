...
public class RemittanceCsvFileFacade {
    ...
    public RemitanceCsvFile generateCsvFile(String referenceComete) {
        LOGGER.info("Preparing Csv file for referenceComete: {}", referenceComete);

        RemittanceOrderPaginationResponse response = remittanceOperationsFacade
                .getOperationListResponse(referenceComete, 0, Integer.MAX_VALUE);
        ...
        generateFilename(referenceComete);
    ...
    public String generateFilename(String referenceComete) {
        return String.format("Remise_%s_%s.csv", referenceComete, LocalDate.now());
    }
